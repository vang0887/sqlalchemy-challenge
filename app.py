...
def stations():
    ...
    # Unravel outcome into a 1D array and convert to a list
...
def stats(start=None, end=None):
    ...
    # selectect statement
    selectect = [func.min(Measurement.tobs), func.avg(Measurement.tobs), func.max(Measurement.tobs)]
    ...
    outcome = session.query(*select).\
        ...
        filter(Measurement.date <= last).all()
    # Unravel outcome into a 1D array and convert to a list
    temps = list(np.ravel(outcome))
    return jsonify(temps)
